...
What genuinely has no backend, and renders an empty or unavailable state rather than invented data: **notifications** (no model or endpoint at all), the dashboard's **Recent Activity**, **Upcoming Deadlines**, **Requiring Your Action** and **Overdue Deliveries** tiles. The **Purchaser Performance** report is in the same position but is currently hidden outright (`hidden: true` in `src/data/reports.ts`) rather than shown as an unavailable card. See `DASHBOARD_BACKEND_GAPS.md` for what each one needs.
